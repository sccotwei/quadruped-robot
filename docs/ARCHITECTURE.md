...
1. `quad-arduino-wifi/quad-arduino-wifi.ino` 中的 `setup()` 初始化 LED、串口、Wi-Fi、HTTP 服务和机器人对象。
2. `robot.init()` 初始化现有舵机控制，随后执行 `robot.home()`。
3. `loop()` 调用 `server.handleClient()` 处理控制请求。
4. `/control` 请求由 `handlePost()` 解析，并交给 `processCommand()`。
5. `processCommand()` 调用 `MiniKame` 的已有动作函数。
...
- 当前运动参数及 GPIO 配置保持不变。
- 现有动作调用为同步执行；是否改为非阻塞架构：TODO，仅在明确授权后评估。
